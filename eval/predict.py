...
sys.path.append("/data1/qyj/ComplexInstruct/")
...
def parse_args():
    ...
    parser.add_argument(
        "--model_name", type=str, help="Your short model name."
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.model_name_or_path is not None:
            ...
            if "messages" in instance:
                ...
                if "request" in instance:
                    prompt = chat_formatting_function(
                        instance["messages"], add_bos=True
                    )
                else:
                    prompt = chat_formatting_function(
                        instance["messages"][:-1], add_bos=True
                    )
                ...
                if args.use_chat_format:
                    ...
                    prompt = chat_formatting_function(messages, add_bos=True)
                else:
                    prompt = "Input:"+instance["prompt"] + "\n\nOutput:"
            elif "instruction" in instance:
                if args.use_chat_format:
                    messages = [{"role": "user", "content": instance["instruction"]}]
                    prompt = chat_formatting_function(messages, add_bos=True)
                else:
        ...
        for instance, output in zip(instances, outputs):
            if "request" in instance:
                instance["request"] = {
                    "result": {"success": True, "completions": output}
                }
            elif "instruction" in instance:
                instance["output"] = output
                instance["generator"] = args.model_name
            else:
