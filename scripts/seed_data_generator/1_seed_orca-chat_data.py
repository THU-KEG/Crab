...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_file",
        type=str,
        default="data/raw_data/orca-chat/orca-chat-gpt4-8k.json",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument("--output_file", type=str, default="1_orca_chat.json")
    ...
    for i, vert in enumerate(data):
        ...
        if vert["conversation"]==[]:
            continue
        try:
            input=vert["instruction"]+"\n\n"+vert["conversation"][0]["input"]
            output=vert["conversation"][0]["output"]
        except:
            print("=========== Case ============", i)
            print(vert)
            break
